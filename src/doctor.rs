...
pub fn doctor_precomputation(name_data: &String, row_index: usize, max_eps: f32, l2: u32) -> (u32, f32){ // b = 0 for integer and b = 1 for flot; p is the precision in the case b = 1. The precision corresponds to the number after the comma. For instance, if we have age = 20.264, p = 3.
...
pub fn doctor_signature_generation_and_storage(path_set: &PathBuf, path_commitment: &PathBuf, path_signature: &PathBuf, path_doctor_public_key: &PathBuf, path_doctor_private_key: &PathBuf){
    ...
    // Sign the commitment using the secret key of the Doctor
...
pub fn doctor_phase(metadata: &Metadata, max_eps: Vec<f32>, path_data: &String, folder_set: &String, folder_opening_key: &String, folder_commitment: &String, folder_proof_commitment: &String, folder_signature: &String, path_doctor_public_key: &PathBuf, path_doctor_private_key: &PathBuf){
	...
	for row_index in 1..=number_row{	
		...
		let precomp = doctor_precomputation(&name_data, row_index, max_eps[row_index-1], l2);
		let l1 = precomp.0;
		//let eps = precomp.1;
