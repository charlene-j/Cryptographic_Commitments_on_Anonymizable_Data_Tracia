...
pub fn doctor_preprocessing(path_data: &String)->(usize,usize,Vec<Metadata>){
    // Open the CSV file.
    let mut reader = csv::Reader::from_path(&path_data).expect("Cannot open the document");
    // Collect the headers.
    let headers = reader.headers().expect("Cannot read the headers").clone();

    // Collect the number of columns.
    let column_number: usize = headers.len();

    // Collect the number of rows.
    let row_number: usize = reader.records().count();

    // Define metadata containing the type of data, the precision, the range...
    let mut dataset_metadata: Vec<Metadata> = Vec::with_capacity(row_number);

    for column_index in 0..column_number{  
        let name_data = &headers[column_index];

        // Since we work with integer, the values are discretized.
        // For example for the serum_creatinine, the value 1.9 will be changed in 19. 
        // (After randomization, the anonymized dataset preserves the floating values).
        let pr = get_precision(&path_data, column_index);

        // Computing the max data in the dataset allows to determine a range for the anonymized data. 
        // Notice that this range determine the maximum value of data that can be taken after randomization. 
        // The range should be fixed by a knowledge expert depending on the type of data (it should not depending on the value of the dataset) 
        // For example for the patient age, it is suitable to fixe max_data = 100, subsequently l_2 = 128).
        // The range can also be taken larger to ensure more privacy.
        let max_data = compute_max(&path_data, column_index, row_number, pr); 

        // l2 corresponds to the ceiling of the base-2 logarithm of max_data. l2 >= log_2(max_data).
        let l_2 = compute_l2(max_data); 

        dataset_metadata.push(Metadata{type_data: name_data.to_string(), col_index: column_index, precision: pr, l2: l_2});
    }
    return (column_number, row_number, dataset_metadata);
}

// Computation of the parameter l1 according to the privacy budget.
pub fn doctor_precomputation(name_data: &String, column_index: usize, row_index: usize, max_eps: f64, l_2: u32, path_eps: &String) -> u32{ // The precision corresponds to the number after the comma. For instance, if we have age = 20.264, pr = 3.

    // Compute the parameter l1 
    let l_1 = compute_l1(l_2, max_eps);

    // Compute the exact privacy budget
    let eps = compute_eps(l_2, l_1);
    //let _= store_float(eps, &path_eps);
    let _ = write_csv_file(path_eps, column_index, row_index, &eps.to_string()); 
...
pub fn doctor_setup_generation_and_storage(l_1: u32, l_2: u32, path_set: &PathBuf){

    let mut csrng = OsRng; // Random Number Generator based on the OS System (Crypto-Secure RNG)
    
    // Setup Operation
    let set = setup(&mut csrng, l_1.try_into().unwrap(), l_2.try_into().unwrap()); 
...
pub fn doctor_commit_generation_and_storage(column_index: usize, row_index: usize, l_1: u32, l_2: u32, pr: u32, path_data: &String, path_set: &PathBuf, path_opening_key: &PathBuf, path_commitment: &PathBuf, path_proof_commitment: &PathBuf){
    ...
    let theta = vec![rand_bitstring(l_1.try_into().unwrap()), rand_bitstring(l_2.try_into().unwrap())];
    ...
    let extracted_c = extract_commit(l_1.try_into().unwrap(), l_2.try_into().unwrap(), &path_commitment); // Collect the stored commitment 
    ...
    let _ = store_proofcommit(&p_com, l_1.try_into().unwrap(), l_2.try_into().unwrap(), &path_proof_commitment);
    ...
    let extracted_pcom = extract_proofcommit(l_1.try_into().unwrap(), l_2.try_into().unwrap(), &path_proof_commitment); // Collect the stored proof of commitment
...
pub fn doctor_signature_generation_and_storage(path_set: &PathBuf, path_commitment: &PathBuf, path_signature: &PathBuf, path_doctor_public_key: &PathBuf, path_doctor_private_key: &PathBuf){
    ...
    let commitment = extract_commit(l_1, l_2, &path_commitment);
...
pub fn doctor_phase(column_number: usize, row_number: usize, dataset_metadata: &Vec<Metadata>, path_data: &String, path_privacy: &String, path_eps: &String, folder_set: &String, folder_opening_key: &String, folder_commitment: &String, folder_proof_commitment: &String, folder_signature: &String, path_doctor_public_key: &PathBuf, path_doctor_private_key: &PathBuf){

    let start_doc_phase = Instant::now();

    let mut reader_privacy = csv::Reader::from_path(&path_privacy).expect("Cannot open the document");

    let privacy_attending: Vec<Vec<String>> = reader_privacy.records().map(|result| {result.expect("Error reading CSV").iter().map(|s| s.to_string()).collect()}).collect();

    // Define some privacy budget (the lower the value, the better the confidentiality, but the lower the utility).
    let eps_highly_sensitive: f64 = 0.5;
    let eps_sensitive: f64 = 0.75;
    let eps_less_sensitive: f64 = 1.2;

    // Create a file to store the privacy budget
    let _= init_csv_file(column_number, row_number, dataset_metadata, path_eps);

    for column_index in 0..column_number{
        // Parse metadata
        let name_data = dataset_metadata[column_index].type_data.clone();
        let pr = dataset_metadata[column_index].precision.clone();
        let l_2 = dataset_metadata[column_index].l2.clone();
        ...
        for row_index in 1..=row_number{
            ...
            if privacy_attending[row_index-1][column_index] == "high"{
                max_eps = eps_highly_sensitive;
            }
            else{
                if privacy_attending[row_index-1][column_index] == "moderate"{
                    max_eps = eps_sensitive;
                }
                else{
                    if privacy_attending[row_index-1][column_index] == "low"{
                        max_eps = eps_less_sensitive; 
                    }
                    else{ // none
                        max_eps = 0f64;
                    }
	        ...
	        let l_1 = doctor_precomputation(&name_data, column_index, row_index, max_eps, l_2, path_eps);
		
	        doctor_setup_generation_and_storage(l_1, l_2, &path_set);
	        doctor_commit_generation_and_storage(column_index, row_index, l_1, l_2, pr, &path_data, &path_set, &path_opening_key, &path_commitment, &path_proof_commitment);
