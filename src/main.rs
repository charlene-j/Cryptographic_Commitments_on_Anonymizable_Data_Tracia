...
println!("Maximum value of the discretized data: {}", max_data);
    ...
    // Create the folder to store the public/private keys of the Doctor.
    ...
    doctor_phase(&metadata, max_eps, &path_data, &fldr_doc_eps_string, &fldr_doc_set_string, &fldr_doc_opening_key_string, &fldr_doc_commitment_string, &fldr_doc_proof_commitment_string, &fldr_doc_signature_string, &path_doc_pub_key, &path_doc_priv_key);
    ...
    let fldr_hosp_eps_string: String = "hospital_space/privacy_budget".to_string();
    ...
    hospital_phase(&metadata, &path_data, &fldr_hosp_eps_string, &fldr_hosp_set_string, &fldr_hosp_opening_key_string, &fldr_hosp_commitment_string, &fldr_hosp_proof_commitment_string, &fldr_hosp_proof_open_string, &fldr_hosp_signature_string, &path_pub_key);
    ...
    let fldr_pub_key = Path::new(&fldr_pub_key_string);
    let path_pub_key = fldr_pub_key.join("doc_pub_key.txt");
    let fldr_res_eps_string: String = "researcher_space/privacy_budget".to_string();
    ...
    researcher_verification(&metadata, &fldr_res_eps_string, &fldr_res_set_string, &fldr_res_commitment_string, &fldr_res_proof_commitment_string, &fldr_res_seed_string, &fldr_res_proof_openldp_string, &fldr_res_signature_string, &path_pub_key, &path_ano_data);  	
}
