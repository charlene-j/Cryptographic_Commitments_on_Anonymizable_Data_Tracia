...
pub fn researcher_privacy_budget_verification(name_data: &String, column_index: usize, row_index: usize, path_eps: &String, path_set: &PathBuf){
...
pub fn researcher_gen_seed(column_number: usize, row_number: usize, dataset_metadata: &Vec<Metadata>, folder_set: &String, folder_seed: &String){
...
pub fn researcher_verify_opening_on_anonymized_data(column_index: usize, row_index: usize, pr: u32, path_set: &PathBuf, path_commitment: &PathBuf, path_seed: &PathBuf, path_proof_opening_ldp: &PathBuf, path_ano_data: &String){
    ...
    let seed = extract_seed(l_1, l_2, path_seed);
    ...
    if pr == 0{
	   ...
	   ano_data = import_data_f64(&path_ano_data, column_index, row_index, pr).expect("Impossible to import the data.");
    }
	
    // Convert the data in binary 
    let bin_ano_data = convert_u32_in_binary(ano_data, l_2.try_into().unwrap());
...
pub fn researcher_verification(column_number: usize, row_number: usize, dataset_metadata: &Vec<Metadata>, path_ano_data: &String, path_eps: &String, folder_set: &String, folder_commitment: &String, folder_proof_commitment: &String, folder_seed: &String, folder_proof_openldp: &String, folder_signature: &String, path_doctor_public_key: &PathBuf){
    ...
    for column_index in 0..column_number{
        ...
        for row_index in 1..=row_number{ 
    	   ...
    	   researcher_privacy_budget_verification(&name_data, column_index, row_index, &path_eps, &path_set);
	       ...
	       researcher_verify_opening_on_anonymized_data(column_index, row_index, pr, &path_set, &path_commitment, &path_seed, &path_proof_openldp, &path_ano_data);
        }
    }
    let duration_res_verification_phase = start_res_verification_phase.elapsed();
    print!("Researcher Verification Phase took {:?} for {} data \n", duration_res_verification_phase, row_number);	
}
